...
from tensorflow.python.keras.layers import LSTM, Dropout, TimeDistributed, Lambda
from tensorflow.python.keras.layers.merge import _Merge, Concatenate
...
class WGANGP(keras.Model, ABC):
    ...
    def build_generator(self):
        ...
        i = Input(shape=self.traj_shape, name='input_traj')
        inputs.append(i)
        inputs.append(noise)
        ...
        lstm_cell = LSTM(units=50,
                         batch_input_shape=(None, self.max_length, 100),
                         return_sequences=True,
                         recurrent_regularizer=l1(0.02))(dropout)
        dropout = Dropout(0.2)(lstm_cell)
        output = TimeDistributed(Dense(1, activation='tanh'), name='output_latlon')(dropout)
        output_stratched = Lambda(lambda x: x * scale_factor)(output)

        model = Model(inputs=inputs, outputs=output_stratched)
        model.summary()
        return model
    ...
    def build_discriminator(self):
        ...
        emb_traj = Lambda(lambda x: tf.stack(x, axis=1))(dense_latlon)
        # LSTM Modeling Layer (many-to-one)
        lstm_cell = LSTM(units=100, recurrent_regularizer=l1(0.02), activation="tanh", return_sequences=True)(emb_traj)
        dropout = Dropout(0.2)(lstm_cell)
        lstm_cell = LSTM(units=50, recurrent_regularizer=l1(0.02), activation="tanh")(dropout)
        dropout = Dropout(0.2)(lstm_cell)
        # Output
